...
Callisto ships with default RPCs — the maintainer's Ganymede Ethereum Mainnet **archive** endpoint (`wss://ganymede.pasiphae.io`, bearer-authenticated) as the auto-connecting primary, with a Flashbots Protect endpoint as a pre-populated **fallback** that Callisto switches to automatically if the primary is unreachable or drops. Both work out of the box; users MUST be able to replace either, disable auto-connect, or add their own endpoints at any time — the defaults are a convenience, not a lock-in. (This supersedes the original requirement that Callisto have no default RPC — a deliberate product decision to give users a working kit immediately while preserving full user control. The default endpoint's bearer token is compiled into release builds as a shared, rate-limited access key — not a per-user secret.)
