...
// Copied verbatim from go-ethereum@v1.17.4 accounts/usbwallet/ledger.go (LGPL-3.0-
// or-later, license/attribution preserved above and unchanged) as part of a local
// fork; see hub.go's package doc comment for why. This driver is transport-
// agnostic (it speaks APDUs over an io.ReadWriter and implements the package's
// driver interface), so it is unmodified — forking it in only lets Callisto stop
// importing upstream go-ethereum usbwallet, which is what drops the ethereum/hid
// dependency in favor of karalabe/usb.
//
...
const (
	...
	ledgerOpSignPersonalMessage ledgerOpcode = 0x08 // Signs an EIP-191 personal message (Callisto: not implemented upstream)
...
// SignMessage implements usbwallet.driver: signs an EIP-191 personal message on
// the Ledger (SIGN_PERSONAL_MESSAGE, opcode 0x08). This is a Callisto addition to
// the driver interface — upstream go-ethereum does not implement personal-message
// signing — used for WalletConnect personal_sign and Safe owner eth_sign.
func (w *ledgerDriver) SignMessage(path accounts.DerivationPath, message []byte) ([]byte, error) {
	if w.offline() {
		return nil, accounts.ErrWalletClosed
	}
...
// ledgerSignPersonalMessage sends a personal message to the Ledger for signing.
// The first chunk carries the derivation path and a 4-byte big-endian message
// length; the message itself is streamed in <=255-byte chunks. The reply is
// v||r||s, returned reordered to r||s||v.
func (w *ledgerDriver) ledgerSignPersonalMessage(derivationPath []uint32, message []byte) ([]byte, error) {
	...
	payload := append(path, msgLen...)
	payload = append(payload, message...)

	var (
		op    = ledgerP1InitTransactionData // 0x00: first chunk
		reply []byte
		err   error
	)
	chunk := 255
	for len(payload) > 0 {
		...
		op = ledgerP1ContTransactionData // 0x80: subsequent chunks
	}
	if len(reply) != crypto.SignatureLength {
		return nil, errors.New("reply lacks signature")
	}
	return append(reply[1:], reply[0]), nil
}
