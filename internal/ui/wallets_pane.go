...
func (p *walletsPane) build() fyne.CanvasObject {
	...
	help := widget.NewLabel("Add hardware wallets and hot wallets here. Import hot wallets via keystore, mneomic, or private key.\n\nHot wallets are encrypted with a passphrase and only decrypted in memory when Unlocked.\n\nDouble-click a wallet to make it active. Manage auto-lock in Settings. Exercise caution with hot wallets.")
